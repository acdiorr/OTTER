...
int main() {
	...
	static const float interleaved[] = {
		// X      Y    Z       R     G     B
		 0.5f, -0.5f, 0.5f,   0.0f, 0.0f, 0.0f, // First Shape
	...
	//My stuff (using the same co-ordinates as the other square because im lazy)
	...
	while (!glfwWindowShouldClose(window)) {
		...
		transform3 = glm::translate(glm::mat4(1.0f), glm::vec3(0, 0.0f, glm::cos(static_cast<float>(thisFrame))));
		transform3 += glm::rotate(glm::mat4(1.0f), static_cast<float>(thisFrame), glm::vec3(0, glm::sin(static_cast<float>(thisFrame)),0));
		
		//transform3 = glm::scale(glm::mat4(1.0f), glm::vec3(0, 4, 0));
		...
		shader->Bind();
		shader->SetUniformMatrix("u_ModelViewProjection", camera->GetViewProjection() * transform);		vao->Bind();
